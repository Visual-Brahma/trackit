...
import { EmailPreferenceForm, NameForm } from "@/components/dashboard/settings/forms";
...
const SettingsPage=async () => {
    ...
    if (!session?.user?.email) {
        ...
        return (
            <div>
                <TypographyP className="text-red-200">Something unexpected happened.</TypographyP>
            </div>
        )
    }
    ...
    if (!profile) {
        ...
        return (
            <div>
                <TypographyP className="text-red-200">Error fetching user profile</TypographyP>
            </div>
        )
    };
    ...
    return (
        <div>
            ...
            <EmailPreferenceForm checked={profile.newsletter} email={session.user.email} />
            <div className="mt-6">
                ...
                <Card className="backdrop-blur-xl backdrop-opacity-60 my-6 bg-destructive/30 ring-2 w-full max-w-md mx-2">
                    <CardContent className="p-4 flex flex-col items-start justify-center space-y-2">
                        <p className="text-sm text-left font-semibold">
                            Deleting your account will remove all your attendance reports, group memberships and notes.
                            This is an irreversible process, think once more before moving ahead.
                        </p>
