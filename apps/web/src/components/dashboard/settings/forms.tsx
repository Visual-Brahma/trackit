...
export const EmailPreferenceForm=({ checked, email }: { checked: boolean, email: string }) => {
    ...
    return (
        <div className="my-4 mt-6">
            <div className="flex items-center space-x-4 w-full justify-between max-w-screen-md">
                ...
                <Switch
                    ...
                    onCheckedChange={async () => {
                        if (await setNewsletterPreference(!newsletter, email)) { setNewsletter(!newsletter); }
                        else {
            ...
            <TypographyP className="max-w-screen-md">
                Control whether to recieve e-mail from Trackit. You will still recieve administrative emails even if this setting is turned off.
            </TypographyP>
        </div>
    )
}
                ...
                }
                {
                    editMode? (
                        <div className="flex items-center justify-center gap-2">
                            <Button className="bg-transparent text-green-200" variant={"outline"} size={"icon"} onClick={async () => {
                            ...
                            <Button className="bg-transparent" variant={"outline"} size={"icon"} onClick={() => {
                                setFullName(name);
                                setEditMode(!editMode);
                            }}>
                                <X />
                        ...
                        (
                            <Button className="bg-transparent" variant={"outline"} size={"icon"} onClick={() => setEditMode(!editMode)}>
                                <SquarePen />
                            </Button>
                        )
                }
